...
monospace baseline and the pinned Omarchy icon font used by the Quattro menu
bar. The icon font can be overridden with
`programs.omanixy.font.icon.package`. It also materializes Tokyo Night's
Quattro background through `~/.local/state/omarchy/current/background` without
enabling a second wallpaper owner.
