...
run_adapter() {
  ...
  env \
    ...
    OMARCHY_SPEEDTEST_SYSFS="${OMARCHY_SPEEDTEST_SYSFS:-}" \
...
if TEST_CLIPBOARD="$test_root/clipboard" TEST_WTYPE="$test_root/wtype" TEST_WL_COPY_HOLD=1 TEST_WTYPE_FAIL=1 \
  HOME="$home" PATH="$bin:$PATH" run_adapter omarchy-menu-emoji-insert "$emoji" 2>"$test_root/error"; then
  printf '%s\n' 'failed emoji paste unexpectedly succeeded' >&2
  exit 1
fi
grep -q 'emoji paste failed' "$test_root/error"
...
test ! -s <(sed -n '1p' "$test_root/nmcli-state")
...
grep -Fqx $'memory\t4.0GB / 8GB' "$test_root/system-stats"
...
mkdir -p "$test_root/speed/fixture0/statistics"
printf '%s\n' 0 > "$test_root/speed/fixture0/statistics/rx_bytes"
printf '%s\n' 0 > "$test_root/speed/fixture0/statistics/tx_bytes"
cat > "$bin/curl" <<'EOF'
#!/usr/bin/env bash
if [[ $* == *api.fast.com* ]]; then
  printf '%s\n' '{"targets":[{"url":"https://speed.test/target"}]}'
  exit 0
fi
exit 1
EOF
chmod +x "$bin/curl"
sed -i "1c#!$(command -v bash)" "$bin/curl"
if OMARCHY_SPEEDTEST_SYSFS="$test_root/speed" HOME="$home" PATH="$bin:$PATH" \
  run_adapter omarchy-network-speedtest down 2>"$test_root/error"; then
  printf '%s\n' 'failed speed-test workers unexpectedly succeeded' >&2
  exit 1
fi
grep -q 'speed-test worker failed' "$test_root/error"
