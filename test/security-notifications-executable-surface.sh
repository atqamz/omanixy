...
# (one per fixed-domain verb) and zero exec/execDetached/run/startDetached
# calls.
scan_output=$("$python_bin" "$scanner" "$service_file" 2>&1)
printf '%s\n' "$scan_output" >&2
grep -Fq "9 command binding(s) passed, executable always 'omanixy-notification-state'" <<<"$scan_output"
